...
// if it is, we send back a token
 ...
 // data.username could be an email or a username
    const user = await UserModel.findOne({$or:[{"login.username":data.username},{"login.email":data.username}]});
    if (!user) {
    ...
    const token = jwt.sign( user.ToClient() );
...
router.get('/', async (req: Request, res: Response) => {
    ...
    res.status(200).send({msg : "user found",token: jwt.sign(user.ToClient()), user: user.ToClient()});
} );
